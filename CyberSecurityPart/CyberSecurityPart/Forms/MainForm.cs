...
{
    /// <summary>
    /// Main chatbot window. Integrates:
    ///   • Existing chat engine, sentiment detection, keyword responses (Parts 1 & 2)
    ///   • NLP keyword routing (Part 3 – Task 2)
    ///   • Task Assistant with MySQL persistence (Part 3 – Task 1)
    ///   • Cybersecurity Quiz mini-game (Part 3 – Task 2)
    ...
    public class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            // Initialise DB (creates schema if missing) — non-blocking
            Task.Run(() =>
            {
        ...
        private void BuildSidebar()
        {
            ...
            logBtn.Click += (s, e) =>
            {
                ...
                AddBotBubble(ActivityLog.Instance.FormatRecent(10));
            ...
            // Add in correct order (Fill last before Bottom-docked)
            _sidebar.Controls.Add(_topicBtns);   // Fill
            _sidebar.Controls.Add(bottomSep);     // Bottom
            _sidebar.Controls.Add(logBtn);        // Bottom
            _sidebar.Controls.Add(quizBtn);       // Bottom
            _sidebar.Controls.Add(taskBtn);       // Bottom
            _sidebar.Controls.Add(heading);       // Top
            _sidebar.Controls.Add(sep1);          // Top
            _sidebar.Controls.Add(_userCard);     // Top
            _sidebar.Controls.Add(logo);          // Top
        }
        ...
        private void ProcessInput()
        {
            ...
            if (!_nameCollected)
            {
                ...
                // Check for due reminders on login
                Task.Run(() =>
                {
                    try
                    {
                        var due = DatabaseHelper.GetDueReminders();
                        if (due.Count > 0)
                        {
                            string list = string.Join("\n",
                                due.Select((t, i) => $"  {i + 1}. {t.Title}"));
                            this.Invoke(() => AddBotBubble(
                                $"⏰ You have {due.Count} overdue task reminder(s):\n{list}\n\n" +
                                "Open the Task Assistant to review them."));
                        }
                    }
                    catch { /* DB might not be configured yet */ }
                });
            ...
            // ── Multi-turn NLP: awaiting task description ─────────────────
            ...
            // ── Multi-turn NLP: awaiting reminder response ────────────────
            ...
            // "Add task …" NLP — detect "add task", "create task", "new task", "remind me to …"
                                     ...
                                     "i need to", "don't let me forget"))
            {
                ...
                if (string.IsNullOrWhiteSpace(title))
                {
                    ...
                    _awaitingTaskDesc = false;   // next message = title not desc
                    // We'll capture title in normal flow; set a special awaiting flag:
        ...
        // ── NLP: awaiting task title when no title was extracted ──────────
        ...
        /// <summary>
        /// Tries to extract a meaningful task title from a natural-language string.
        /// Handles: "add task to enable 2fa", "remind me to update my password", etc.
        /// </summary>
        ...
        /// <summary>
        /// Parses a plain-language reminder timeframe into a concrete DateTime.
        /// Handles: "7", "7 days", "in 3 days", "tomorrow", "next week", "15 july", etc.
        /// </summary>

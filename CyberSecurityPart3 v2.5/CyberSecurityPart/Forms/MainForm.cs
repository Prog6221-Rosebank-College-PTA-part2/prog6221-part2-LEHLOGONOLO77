...
namespace CyberSecurityAwarenessBot.Forms
{
    ...
    public class MainForm : Form
    {
        ...
        // ── Reminder system ─────────────────────────────────────────────
        private readonly System.Windows.Forms.Timer _reminderTimer = new() { Interval = 60_000 }; // checks every 60s
        ...
        public MainForm()
        {
            ...
            // Every minute, while the app is open, silently re-checks the
            // database for tasks whose reminder date has arrived and have
            // not been flagged yet, then pops a tray balloon + chat message.
        ...
        private void ProcessInput()
        {
            ...
            if (!_nameCollected)
            {
                ...
                // Check for due reminders on login (reuses the same tracked
                // notification path as the periodic timer, so nothing repeats)
                CheckReminders();

...
namespace CyberSecurityAwarenessBot.Responses
{
    ...
    public class ResponseEngine
    {
        ...
        public string GetResponse(string input, string name, UserMemory memory)
        {
            ...
            {
                int score = ScoreTopic(lower, words, t);
                if (score > bestScore)
                {
                    bestScore = score;
                    best = t;
            ...
            {
                LastDetectedTopic = best.Name;
                string r = best.Responses[_rng.Next(best.Responses.Length)].Replace("{n}", name);
                string recall = memory.GetMemoryRecall();
                if (!string.IsNullOrEmpty(recall) && memory.FavouriteTopic != best.Name)
                    r += $"\n\n  Tip: {recall}";
                return r;
            }
        ...
        /// <summary>
        /// Scores how well a topic matches the input: an exact phrase match scores
        /// highest, but a single mistyped word (e.g. "phissing", "passwrod") still
        /// scores a hit via Levenshtein distance — this is what lets the bot adapt
        /// to imperfect, natural phrasing instead of requiring exact keywords.
        /// </summary>
        private static int ScoreTopic(string lowerInput, string[] words, Topic t)
        {
            int score = 0;
            foreach (string keyword in t.Keywords)
            {
                if (lowerInput.Contains(keyword))
                {
                    score += keyword.Contains(' ') ? 3 : 2; // multi-word phrase = stronger signal
                    continue;
                }

                if (keyword.Contains(' ')) continue; // only fuzzy-match single words

                foreach (string w in words)
                {
                    if (w.Length < 4 || keyword.Length < 4) continue;
                    if (LevenshteinDistance(w, keyword) == 1)
                    {
                        score += 1; // near-miss / typo still counts, but weaker than an exact hit
                        break;
                    }
                }
            }
        ...
        /// <summary>Classic edit-distance algorithm used to tolerate single-character typos.</summary>

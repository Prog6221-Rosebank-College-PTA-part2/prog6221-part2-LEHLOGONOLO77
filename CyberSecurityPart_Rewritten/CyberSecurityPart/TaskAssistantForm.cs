...
{
    /// cybersecurity tasks with optional date reminders 
    public class TaskAssistantForm : Form
    {
        ...
        // ── FIXED: safe enum parse so "None" or unknown values don't crash ───────
        private static TaskStatus ParseStatus(string raw)
        {
            // Normalise: trim whitespace, handle null/empty
            if (string.IsNullOrWhiteSpace(raw))
                return TaskStatus.Pending;

            // Case-insensitive match; fall back to Pending for "None" or anything unrecognised
            if (Enum.TryParse<TaskStatus>(raw.Trim(), ignoreCase: true, out var result))
                return result;
        ...
        private void LoadTasks()
        {
            ...
            try
            {
                ...
                {
                    // ── FIXED: guard against null/invalid Status on each row ──
                    TaskStatus safeStatus = ParseStatus(t.Status.ToString());
                    // ──────────────────────────────────────────────────────────
                    ...
                    // ── FIXED: use safeStatus instead of t.Status directly ───
                    item.ForeColor = safeStatus == TaskStatus.Completed
